...
#comb_m = data.frame(matrix(0, nrow = length(all_sag), ncol = length(all_vp)))
...
#comb_l_agg = aggregate(value ~ Var1 + Var2, data = comb_l, sum)

sag_spe_raw = read.csv("new_classification.csv", header = T)
sag_spe = data.frame(sag_id = substr(sag_spe_raw$SAG, 8, 10), spe = sag_spe_raw$species)

# comb_l_agg_spe = merge(comb_l_agg, sag_spe, by.x = "Var1", by.y = "sag_id")
# comb_l_agg_spe_agg = aggregate(value ~ spe + Var2, data = comb_l_agg_spe, sum)
# comb_m_spe_level = dcast(comb_l_agg_spe_agg, spe ~ Var2, fill = 0)
...
merged_sub_data = as.matrix(comb_m_spe[-which(rownames(comb_m_spe)=="Unknown"),])

sag_spe_count = data.frame(table(sag_spe_raw$species))
...
merged_sub_data_color = sapply(rownames(merged_sub_data),get_color)
...
column_hostcnt_cnt = apply(merged_sub_data_norm_0_converted_colname_corrected, 2, function(x){sum(!is.na(x))})
...
#column_comb_cnt_order_string = paste0(colnames(column_comb_cnt_order), " (", column_comb_cnt_order[1,], ")")
...
roworder = read.table("heatmap_row_order.txt", header=F, sep=",")
...
pdf("heatmap_species_level_no_unknown_combine_inf_crispr.pdf", height = 6, width = 10)
