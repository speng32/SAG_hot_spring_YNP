...
rownames(sag_vp_inf_m) = sag_vp_inf_m[,1]
...
################# lower 2 reads 300bp #################
sag_vp_inf_m[sag_vp_inf_m < 300] = 0
keep_rows = rowSums(sag_vp_inf_m) > 0
keep_cols = colSums(sag_vp_inf_m) > 0
sag_vp_inf_m = sag_vp_inf_m[keep_rows, keep_cols]
...
sag_vp = read.csv("../matrix_infection_by_bp.csv", na.strings = 0)
sag_vp[sag_vp<300] = NA
sag_vp = melt(sag_vp, variable.name = "x", na.rm = T)
sag_vp[,2] = gsub("X", "", sag_vp[,2])
sag_vp[, ncol(sag_vp)+1] = sag_vp[,1]
...
# final_rownames = row.names(merged_sub_data_norm_0_converted_colname_corrected_rearranged_complex_colreorder)
# cnt_rows = table(comb_l_spe[!duplicated(comb_l_spe[, 1]), ][, 3])
# cnt_rows_t = data.frame(spe=names(cnt_rows), update_name = paste0(names(cnt_rows), " (", cnt_rows, ")"))
# rownames(merged_sub_data_norm_0_converted_colname_corrected_rearranged_complex_colreorder) = cnt_rows_t[match(final_rownames, cnt_rows_t[, 1]), 2]

merged_sub_data_color_rearranged = merged_sub_data_color[rearrange_order]



pdf("fig3_class1_only_raw_lowerbound_300bp.pdf", height = 6, width = 10)
...
################# upper 5 reads 750bp #################
sag_vp_inf_m[sag_vp_inf_m < 750] = 0
keep_rows = rowSums(sag_vp_inf_m) > 0
keep_cols = colSums(sag_vp_inf_m) > 0
sag_vp_inf_m = sag_vp_inf_m[keep_rows, keep_cols]
...
pdf("fig3_class1_only_raw_upperbound_750bp.pdf", height = 6, width = 10)
